...
<?php
				    ...
				    $sql1= "select * from class_fare where class_name='$classs'";
					$data1=mysqli_query($con,$sql1);
					$row1=mysqli_fetch_array($data1);
					$cid=$row1['class_id'];
				
					
					$sql2= "select * from meal where meal_name='$meal'";
					$data2=mysqli_query($con,$sql2);
					$row2=mysqli_fetch_array($data2);
					$mid=$row2['meal_id'];
				
                    
					$sql3= "select * from seats where seat_nam='$seat'";
					$data3=mysqli_query($con,$sql3);
					$row3=mysqli_fetch_array($data3);
					$sid=$row3['s_id'];
					
					 
					 $sql = "UPDATE `user_flight_info` SET `c_id`='$cid',`mel_id`='$mid',`seat_id`='$sid' WHERE Member_id='$memid' && ser_id='$serid'";
        ...
        <div class="row no-gutters slider-text justify-content-start align-items-center">
          ...
          <div class="col-lg-4 col-md-6 mt-0 mt-md-5 d-flex">
          	<form action="" method="Post" class="request-form ftco-animate">
			   ...
			   <div class="form-group">
                <label for="" class="label">Seat Type</label>
				 <select name="seat"  class="form-control">
                            	<option value="none">Select Seat Type</option>
                              <option value="Window seat">Window seats</option>
		                        <option value="Aisle seat">Aisle seats</option>
		                        <option value="Middle seat">Middle seats</option>
				                <option value="Exit row seat">Exit row seats</option>
		                        <option value="Bulkhead seat">Bulkhead seats</option>

								</select>
              </div>
            
			      <div class="form-group">
                <label for="" class="label">Type of Meal</label>
